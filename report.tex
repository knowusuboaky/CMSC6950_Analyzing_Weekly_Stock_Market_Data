...
It appears as though $\textbf{56.2\%}$ of the responses are predicted correctly (model accuracy). In other words, $\textbf{43.8\%}$ is the training error rate, which is often overly optimistic. The precision is \textbf{56.2\%} which tells that we are able to perform better than baseline.\\\\
...
\subsubsection{Logistic Regression (Interation Predictors)} 
